...
locals {
    ...
    static-ip-LAN = "172.18.3.40"
    static-ip-WAN = "172.18.3.70"
}
...
resource "azurerm_linux_virtual_machine" "snatvm" {
  ...
  location              = var.location
  resource_group_name   = azurerm_resource_group.baserg.name
  network_interface_ids = [azurerm_network_interface.snatvm-LAN.id, azurerm_network_interface.snatvm-WAN.id]
